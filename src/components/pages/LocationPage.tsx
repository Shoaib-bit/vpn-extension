    ...
    <div className='min-h-[400px] bg-vpn-background  flex flex-col p-3 max-w-md '>
      ...
      <div className='mt-2 bg-black/10 rounded-lg overflow-hidden flex-1'>
        <Tabs defaultValue='all' value={activeTab} onValueChange={handleTabChange} className='w-full'>
          <TabsList className='w-full grid grid-cols-2 bg-black/20 rounded-none border-b border-gray-800'>
            <TabsTrigger
              value='all'
              className='data-[state=active]:bg-black/10 data-[state=active]:shadow-none rounded-none'
            >
              All Servers
            </TabsTrigger>
            <TabsTrigger
          ...
          <TabsContent value='all' className='m-0'>
            <LocationList searchQuery={searchQuery} showFavoritesOnly={false} />
          </TabsContent>

          <TabsContent value='favorites' className='m-0'>
            <LocationList searchQuery={searchQuery} showFavoritesOnly={true} />
          </TabsContent>
        </Tabs>
      </div>
